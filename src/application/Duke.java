...
public class Duke extends ImageView { // duke details 
    private double x, y;
    ...
    //image assets
    private final Image standImage = new Image(getClass().getResource("stand_duke.png").toExternalForm());
    private final Image walkLeft1 = new Image(getClass().getResource("duke_left1.png").toExternalForm());
    private final Image walkLeft2 = new Image(getClass().getResource("duke_left2.png").toExternalForm());
    private final Image walkRight1 = new Image(getClass().getResource("duke_right1.png").toExternalForm());
    private final Image walkRight2 = new Image(getClass().getResource("duke_right2.png").toExternalForm());
    private final Image standImageKey = new Image(getClass().getResource("duke_stand_key.png").toExternalForm());
    private final Image walkLeftKey1 = new Image(getClass().getResource("duke_left_key1.png").toExternalForm());
    private final Image walkLeftKey2 = new Image(getClass().getResource("duke_left_key2.png").toExternalForm());
    private final Image walkRightKey1 = new Image(getClass().getResource("duke_right_key1.png").toExternalForm());
    private final Image walkRightKey2 = new Image(getClass().getResource("duke_right_key2.png").toExternalForm());
    ...
    public Duke(double startX, double startY, WholeMap map) { // initialize duke into the map 
        this.x = startX;
        this.y = startY;
        this.map = map;
    ...
    public void moveX(double dx) { 
        ...
        double newX = clamp(x + dx, 0, map.getMapWidth() - getFitWidth()); // prevent duke from going out of bounds horizontally
        ...
        for (int row = top; row <= bottom; row++) { // duke interactions with other objects 
            for (int col = left; col <= right; col++) {
                Tile tile = map.getTileAt(col, row);
                if (tile != null) {
                    if (tile.type == 'Y') openDukeDoor(tile);
        ...
        animationCounter++; // duke walking animation toggle
    ...
    public void applyGravity() { // gravity and vertical collision 
        ...
        if (velocityY > 0) { // handle falling 
            ...
            for (int col = left; col <= right; col++) {
                ...
                if (groundTile != null) {
                    if (groundTile.type == 'Y') openDukeDoor(groundTile);
        ...
        if (newY + getFitHeight() > map.getMapHeight()) { // prevent duke from jumping out of bounds
    ...
    public void stopMoving() { // update duke img after stop moving 
        if (!isAlive || reachedDoor) return;
        updateStandImage();
    }
    ...
    private void openDukeDoor(Tile doorTile) { // open door 
    ...
    private void fadeOut() {
        setVisible(false); // fade animation
    }
